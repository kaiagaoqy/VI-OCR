...
if __name__ == "__main__":
    ...
    import pandas as pd
    import tqdm
    FOLDER = 'etdrs_word' # totaltext, mnread, fonts, etdrs, totaltext_all
    jsonl_files = glob.glob(f'output/{FOLDER}/*.jsonl')
    for jsonl_file in jsonl_files:
        json_file = jsonl_file.replace('.jsonl', '.json')
        jsonl_to_json(jsonl_file, json_file)
    files = glob.glob(f'output/{FOLDER}/*.json')
